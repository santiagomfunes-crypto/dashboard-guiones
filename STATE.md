...
**Pedido al propietario (copy-paste / WhatsApp)** — jul 2026: en el modal de cada propiedad hay dos botones (`copiarPedido()` / `waPedido()`, dorado y verde WA). Arman un mensaje con la documentación que TODAVÍA falta, en tono Santi. `Copiar` usa clipboard API con fallback execCommand; `WhatsApp` abre `wa.me/?text=` (sin número, Santi elige el contacto — se piden manualmente). Lógica en `buildPedido()`.
  - **Solo se pide lo que aporta el propietario.** Cada item del CHECKLIST tiene flag `pedir` (bool) + `pedirLabel` (override de texto). `pedir:false` = lo gestiona Santi/escribanía → se trackea en la página pero NO entra en el pedido. Verificado contra fuente oficial PBA (gba.gob.ar): certificado de dominio (RPI) e informe de inhibición los solicita el ESCRIBANO tras firmar el boleto; amojonamiento = agrimensor; libre deudas de servicios (EDEA/Camuzzi/OSSE) se tramitan con nº de cuenta. Todo eso quedó `pedir:false`. Al dueño se le piden: escritura, DNI, planos si los tiene, boletas de inmobiliario/tasa, y en depto reglamento + libre deuda expensas + datos de administración + autorización de venta.
  - Mensaje reescrito con profundidad: abre con el *por qué* (armar carpeta para cerrar rápido, no perder la operación por papeles tardíos), lista limpia sin títulos de categoría, aclara que los certificados registrales los gestiona Santi, y cierra sin presión ("aunque sea una foto", "sin apuro"). El filtro sigue siendo estado ≠ recibido y ≠ no_aplica → propiedad nueva lista todo lo pedible, avanzada solo lo pendiente.
